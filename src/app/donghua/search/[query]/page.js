...
export default function DonghuaSearchPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch(getDonghuaUrl(`search/${encodeURIComponent(searchQuery)}${currentPage > 1 ? `/${currentPage}` : ''}`), {
          cache: "no-store",
        });
        ...
        <div className="container mx-auto p-4 py-12">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              {pagination 
                  ? `Halaman ${pagination.current_page} dari ${pagination.total_pages} · ${pagination.total_items || (searchResults.length * pagination.total_pages)}+ donghua ditemukan` 
